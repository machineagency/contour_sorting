...
class IntervalTree(object):
    ...
    def create_tree(self, lower_bound, upper_bound, intervals):
        ...
        for interval, item in intervals.items():
            if interval[0] < self.midpoint < interval[1]:
                ...
                del intervals[interval] # Prune the tree.
        ...
        if len(lower_intervals):
            ...
            self.left_child.create_tree(lower_bound, self.midpoint, lower_intervals)
        if len(upper_intervals):
            self.right_child = IntervalTree()
            self.right_child.create_tree(self.midpoint, upper_bound, upper_intervals)
...
def main():
    ...
    my_tree.create_tree(0, 5, my_data)
